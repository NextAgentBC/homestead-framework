...
PERSONA = """\
你是「{site_name}」网站的 AI 在线客服助手，名字叫「小爪」。你正在和一位**匿名网站访客**对话。
...
def _site_facts() -> str:
    ...
    lines = [
        f"- 网站名称：{cfg['SITE_NAME']}",
        f"- 行业/定位：{cfg.get('SITE_INDUSTRY','')}，面向 {cfg.get('SITE_AUDIENCE','')}",
    ]
    try:
        pages = (
...
def build_prompt(history: list[dict], message: str, locale: str | None = None) -> str:
    ...
    cfg = current_app.config
    persona = PERSONA.format(site_name=cfg["SITE_NAME"])
    turns = cfg.get("WEBCHAT_HISTORY_TURNS", 12)
    convo_lines = []
    for m in history[-turns:]:
        who = "访客" if m.get("role") == "visitor" else "小爪"
