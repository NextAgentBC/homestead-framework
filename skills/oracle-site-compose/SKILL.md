...
- Available block types: `hero` · `stats` · `logos` · `features` · `problem` · `comparison` · `testimonials` · `pricing` · `faq` · `cta` · `steps` (numbered how-it-works) · `gallery` (image grid) · `team` (member cards) · `banner` (highlight strip) · `section` (the flexible token-driven block — see `../oracle-site-capture/SKILL.md`). Image-bearing blocks (`gallery`, `team`, `testimonials` avatars, `section` media) take a `/api/media/<file>` URL — **upload the photo first** via `../oracle-site-media/SKILL.md`. The catalog is the source of truth; new types appear there as they're added.
