...
# Homestead — i18n (you are the translator)
...
```bash
curl -s "$HOMESTEAD_SITE_API/site" | jq '{locales, defaultLocale}'   # e.g. {"locales":["en","zh"],"defaultLocale":"en"}
```
...
curl -s "$HOMESTEAD_SITE_API/admin/compose/home/blocks" -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN" | jq '.item.blocks'
# translate a block into zh (content deep-merges; only the fields you send change):
curl -s -X PATCH "$HOMESTEAD_SITE_API/admin/compose/home/blocks/b_xxxx?locale=zh" \
...
```bash
curl -s -X PATCH "$HOMESTEAD_SITE_API/admin/pages/<id>?locale=zh" -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN" -H "Content-Type: application/json" \
...
```bash
curl -s -X PATCH "$HOMESTEAD_SITE_API/admin/blogs/<id>?locale=zh" -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN" -H "Content-Type: application/json" \
...
```bash
curl -s "$HOMESTEAD_SITE_API/i18n/zh" | jq '.item.messages'      # current zh overrides (public)
curl -s -X PATCH "$HOMESTEAD_SITE_API/admin/i18n/zh" -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN" -H "Content-Type: application/json" \
...
- Don't touch tokens here (global). Use `../homestead-site-design` for look, this for language.
