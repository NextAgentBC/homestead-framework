---
name: homestead-site-pages
description: "Create and manage standalone content pages (About, Services, Privacy, FAQ…) on the Homestead over its API — they render instantly with NO rebuild and auto-appear in the nav. Triggers: '做一个 About 页面 / 加个关于我们页', 'make an about us page', '新建页面 / 加一页', 'create a page', '改 about 页 / 更新页面', '删页面', 'services 页 / 隐私政策页'."
metadata:
  version: 0.1.0
  openclaw:
    category: "website"
...
```bash
curl -s "$HOMESTEAD_SITE_API/pages"          # published pages (nav): slug, navLabel, navOrder, showInNav
curl -s "$HOMESTEAD_SITE_API/pages/<slug>"   # one published page incl. bodyMarkdown
```
...
curl -s -X POST "$HOMESTEAD_SITE_API/admin/pages" \
...
curl -s -X PATCH "$HOMESTEAD_SITE_API/admin/pages/<id>" \
  -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN" -H "Content-Type: application/json" \
  -d '{"status": "published", "nav_order": 20}'

# Delete a page
curl -s -X DELETE "$HOMESTEAD_SITE_API/admin/pages/<id>" \
  -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN"
```

## Module-composed pages (landing pages)

A page can be built from **design modules** (hero, features, pricing, faq, cta…) — the same modules as the home — instead of markdown. Pass `sections` instead of `body_markdown`; see `../homestead-site-design/SKILL.md` for the module catalog and content shapes.
...
> Industry recipes pair well with the matching **theme** preset — e.g. scaffold `restaurant` + `generate {"preset":"restaurant"}` (see `../homestead-site-design`).

```bash
curl -s "$HOMESTEAD_SITE_API/page-templates"          # browse the recipes (public)
# scaffold + publish a Services page, then tweak any block via the compose skill
curl -s -X POST "$HOMESTEAD_SITE_API/admin/pages" -H "Authorization: Bearer $HOMESTEAD_SITE_TOKEN" -H "Content-Type: application/json" \
...
`../homestead-site-compose/SKILL.md` and translate with `?locale=zh`. **contact** + **blog** aren't here —
