...
# Oracle Site — Media (upload a photo, use it on the site)

> Prerequisite: `../oracle-site-shared/SKILL.md` for `$ORACLE_SITE_API` + `$ORACLE_SITE_TOKEN`.
> Structure edits go through `../oracle-site-compose/SKILL.md`.
...
- **A screenshot of some website's section the user wants to *replicate*** → **do not upload it.**
  ...
  `../oracle-site-capture/SKILL.md`. That's assimilation (your colors/fonts), not a pixel copy.
...
curl -s -X POST "$ORACLE_SITE_API/admin/media" \
...
```bash
URL=$(curl -s -X POST "$ORACLE_SITE_API/admin/media" -H "Authorization: Bearer $ORACLE_SITE_TOKEN" -F "file=@photo.jpg" | jq -r '.item.url')
```
...
```bash
curl -s "$ORACLE_SITE_API/admin/media" -H "Authorization: Bearer $ORACLE_SITE_TOKEN" | jq '.items'   # list (name, url, bytes)
curl -s -X DELETE "$ORACLE_SITE_API/admin/media/<filename>" -H "Authorization: Bearer $ORACLE_SITE_TOKEN"  # remove one
```
...
- A *screenshot to replicate a layout* is a capture job, not an upload — `../oracle-site-capture/SKILL.md`.
