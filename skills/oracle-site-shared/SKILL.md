...
description: "Homestead (the website framework) API basics + auth + capability map — READ FIRST. Base URL, admin token issuance, conventions, the whole API/theme/block surface at a glance, plus site config, health, OpenAPI, Google login. Triggers: 'oracle 网站 / homestead / 网站 API', 'site health 状态', '拿/签发 admin token', 'admin token', '网站信息 / site info', 'openapi 契约', '网站有哪些能力 / api 清单'."
metadata:
  version: 0.2.0
  openclaw:
    category: "website"
...
```bash
export ORACLE_SITE_API="https://homestead-api.nextagent.ca/api"   # this deployment's API base
```
...
- **Content:** `oracle-site-blog` · `…-pages` · `…-newsletter`.
- **Design:** `oracle-site-design` (theme/tokens + the 18 templates) · `…-compose` (block-level page editing) ·
  `…-capture` (rebuild a section from a screenshot — flexible `section` block + `/patterns` library).
- **Media:** `oracle-site-media` (upload a photo → host it → drop into a gallery/team/hero/blog; upload-only).
- **Language:** `oracle-site-i18n` (translate content + chrome, path-based `/zh`). **Ops:** `oracle-site-ops` (deploy/status).
